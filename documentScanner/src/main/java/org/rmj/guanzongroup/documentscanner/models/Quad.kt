package org.rmj.guanzongroup.documentscanner.models

import android.graphics.PointF
import android.graphics.RectF
import org.opencv.core.Point
import org.rmj.guanzongroup.documentscanner.enums.QuadCorner
...
import org.rmj.guanzongroup.documentscanner.extensions.multiply
import org.rmj.guanzongroup.documentscanner.extensions.toPointF
...
class Quad(
  ...
  /**
   * This finds the corner that's closest to a point. When a user touches to drag the cropper, that
   * point is used to determine which corner to move.
   *
  ...
  fun mapOriginalToPreviewImageCoordinates(imagePreviewBounds: RectF, ratio: Float): Quad {
    return Quad(
      ...
      topRightCorner.multiply(ratio).move(imagePreviewBounds.left, imagePreviewBounds.top),
      bottomRightCorner.multiply(ratio).move(imagePreviewBounds.left, imagePreviewBounds.top),
      bottomLeftCorner.multiply(ratio).move(imagePreviewBounds.left, imagePreviewBounds.top)
    )
  }
  ...
  fun mapPreviewToOriginalImageCoordinates(imagePreviewBounds: RectF, ratio: Float): Quad {
    return Quad(
      topLeftCorner.move(-imagePreviewBounds.left, -imagePreviewBounds.top).multiply(1 / ratio),
      topRightCorner.move(-imagePreviewBounds.left, -imagePreviewBounds.top).multiply(1 / ratio),
      bottomRightCorner.move(-imagePreviewBounds.left, -imagePreviewBounds.top).multiply(1 / ratio),
      bottomLeftCorner.move(-imagePreviewBounds.left, -imagePreviewBounds.top).multiply(1 / ratio)
    )
  }
}
