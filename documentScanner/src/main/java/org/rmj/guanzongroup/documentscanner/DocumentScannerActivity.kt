...
import org.rmj.guanzongroup.documentscanner.DocumentDetector
import org.rmj.guanzongroup.documentscanner.constants.DefaultSetting
import org.rmj.guanzongroup.documentscanner.constants.DocumentScannerExtra
import org.rmj.guanzongroup.documentscanner.extensions.move
import org.rmj.guanzongroup.documentscanner.extensions.onClick
import org.rmj.guanzongroup.documentscanner.extensions.saveToFile
import org.rmj.guanzongroup.documentscanner.extensions.screenWidth
import org.rmj.guanzongroup.documentscanner.models.Document
import org.rmj.guanzongroup.documentscanner.models.Quad
import org.rmj.guanzongroup.documentscanner.ui.ImageCropView
import org.rmj.guanzongroup.documentscanner.utils.CameraUtil
import org.rmj.guanzongroup.documentscanner.utils.FileUtil
import org.rmj.guanzongroup.documentscanner.utils.ImageUtil
...
class DocumentScannerActivity : AppCompatActivity() {
  ...
  /**
   * @property cropperOffsetWhenCornersNotFound if we can't find document corners, we set corners to
   * image size with a slight margin
   */
  ...
  /**
   * @property cameraUtil gets called with photo file path once user takes photo, or exits camera
   */
   ...
   * original photo path and 4 document corners to documents list. If user isn't allowed to adjust
   * corners, call this automatically.
   */
  ...
  /**
   * This gets called when a user presses the new document button. Store current photo path with
   * document corners. Then open the camera, so user can take a photo of the next page or document
   */
  ...
  /**
   * This gets called when a user presses the done button. Store current photo path with document
   * corners. Then crop document using corners, and return cropped image paths
   */
  ...
  /**
   * This gets called when a user presses the retake photo button. The user presses this in case the
   * original document photo isn't good, and they need to take it again.
   */
  ...
  /**
   * This gets called when a user doesn't want to complete the document scan after starting. For
   * example a user can quit out of the camera before snapping a photo of the document.
   */
