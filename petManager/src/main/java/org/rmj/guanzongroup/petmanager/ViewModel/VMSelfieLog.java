...
//        new OnBranchCheckTask(instance, callback).execute(BranchCde);
        TaskExecutor.Execute(callback, new OnTaskExecuteListener() {
            ...
            @Override
            public Object DoInBackground(Object args) {
                if (!poLog.ValidateExistingBranch(BranchCde)) {
                    message = poLog.getMessage();
                    return false;
                }
...
//        new TimeInTask(instance, callback).execute(foVal);
        TaskExecutor.Execute(callback, new OnTaskExecuteListener() {
            ...
            @Override
            public Object DoInBackground(Object args) {
//                SelfieLog.SelfieLogDetail lsSelfieLogDetail = (SelfieLog.SelfieLogDetail)args;
                try {
                    String lsTransNo = poLog.SaveSelfieLog(foVal);
                    if (lsTransNo == null) {
                        message = poLog.getMessage();
                        return 0;
                    }
...
//        new ValidateUserCashCount(callback).execute(fsVal);
        TaskExecutor.Execute(callback, new OnTaskExecuteListener() {
            ...
            @Override
            public Object DoInBackground(Object args) {
                int lnResult = poCash.ValidateCashCount(fsVal);
                if (lnResult == 1) {
                    message = fsVal;
                    return lnResult;
                }
...
private static class ValidateSelfieBranch extends AsyncTask<String, Void, Integer> {
    ...
    private final Application instance;
    private final VMSelfieLog.OnValidateSelfieBranch listener;

    private final SelfieLog poSys;

    private String message;

    public ValidateSelfieBranch(Application instance, VMSelfieLog.OnValidateSelfieBranch listener) {
        this.instance = instance;
